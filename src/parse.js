...
const ANCHOR = '(?:\\/|^)';
const DOT_LITERAL = '\\.';
const DOTS_SLASH = '\\.{1,2}(?:$|\\/)';
const NO_DOT = '(?!\\.)';
const NO_DOT_SLASH = '(?!\\.{0,2}(?:$|\\/))';
const NO_DOTS = `(?!${ANCHOR}${DOTS_SLASH})`;
const ONE_CHAR = '(?=.)';
const PLUS_LITERAL = '\\+';
const QMARK = '[^/]';
const QMARK_LITERAL = '\\?';
...
const globstar = dot => {
  return `(?:(?!${ANCHOR}${dot ? DOTS_SLASH : DOT_LITERAL}).)*?`;
};
...
const fastpaths = (input, options) => {
  ...
  let starPlus = win32 ? STAR_PLUS_WINDOWS : STAR_PLUS;
  let star = win32 ? STAR_WINDOWS : STAR;
  let slashDot = opts.dot ? (opts.dots ? NO_DOTS : NO_DOT_SLASH) : NO_DOT;
  let dot = opts.dot ? NO_DOTS : NO_DOT;

  switch (input) {
    case '*':
      return `${dot}${ONE_CHAR}${star}`;

    case '.*':
      return `\\.${ONE_CHAR}${star}`;

    case '*.*':
      return `${dot}${star}\\.${ONE_CHAR}${star}`;

    case '*/*':
      return `${dot}${star}${SLASH_LITERAL}${ONE_CHAR}${slashDot}${star}`;

    case '**':
      return dot + globstar(opts.dot);
...
const parse = (input, options) => {
  ...
  let dot = opts.dot ? (opts.dots ? NO_DOTS : NO_DOT_SLASH) : NO_DOT;
  ...
  while (!eos()) {
    ...
    if (value === '/') {
      ...
      if (prev.type === 'globstar' && prev.prev && !input.slice(state.index + 1).startsWith('/*')) {

        prev.prev.output = '(?:' + prev.prev.output;
        if (prev.prev.type === 'bos') {
          output += `|)`;
        } else {
          let dotPath = input.slice(state.index + 1).startsWith('..');
          output = `|${dotPath ? '' : dot})` + output;
        }
      }
    ...
    if (state.index === start || prev.type === 'slash' || prev.type === 'dot') {
      if (prev.type === 'dot' && opts.dots !== true)  {
        // force NO_DOT_SLASH
        push({ type: 'nodot', value: '', output: NO_DOT_SLASH });
      } else {
        push({ type: 'nodot', value: '', output: dot });
      }
