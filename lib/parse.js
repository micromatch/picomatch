...
parse.fastpaths = (input, options) => {
  ...
  const create = str => {
    // `noglobstar` makes `**` a single star, so a leading `**/` is a required
    // segment rather than the optional one the cases below emit.
    if (opts.noglobstar === true && str.startsWith('**/')) return;
