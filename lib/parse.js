...
const parse = (input, options) => {
  ...
  // let nodot = opts.dot ? (opts.dots ? NO_DOTS : '') : NO_DOT;
  ...
  while (!eos()) {
    ...
    if (value === '/') {
      ...
      if (prev.type === 'globstar' && prev.prior) {
        if (!input.slice(state.index + 1).startsWith('/*')) {
    ...
    if (state.index === start || prev.type === 'slash' || prev.type === 'dot') {
      if (prev.type === 'dot') {
        push({ type: 'nodot', value: '', output: '(?!\\.{0,1}($|\\/))' });
      } else if (opts.dot === true) {
        push({ type: 'nodot', value: '', output: '(?!\\.{1,2}($|\\/))' });
      } else {
