...
console.log(globstar(true))
console.log(globstar(false))
...
const parse = (input, options) => {
  ...
  let star = opts.bash === true ? '.*?' : (win32 ? STAR_WINDOWS : STAR);
  let qmark = opts.bash === true ? '.' : (win32 ? QMARK_WINDOWS : QMARK);
  let qmarkNoDot = (opts.dot || opts.bash) ? qmark : (win32 ? QMARK_WINDOWS_NO_DOT : QMARK_NO_DOT);
  ...
  if (opts.fastpaths !== false && input[0] !== '*' && !/[/{[()\]}"]/.test(input)) {
    if (opts.nonegate !== true && input[0] === '!') {
      state.index++;
      negate(state);
      input = input.slice(state.index + 1);
    }

    let backslashes = false;

    state.output = input
      .replace(SPECIAL_CHAR_REGEX, (m, esc, chars, first, rest, index) => {
        if (first === '\\') {
          backslashes = true;
          return m;
        }
        ...
        if (first === '*') {
          ...
          return ((index === 0 && state.negated) ? nodot : '') + star;
        }
        return esc ? m : '\\' + m;
      });
    ...
    if (backslashes === true) {
      if (opts.unescape === true) {
        ...
        state.output = state.output.replace(/\\+/g, m => {
  ...
  while (!eos()) {
    ...
    if (value === '/') {
      ...
      push({ type: 'slash', value, output: win32 ? SLASH_LITERAL_WINDOWS : SLASH_LITERAL });
      continue;
    }
    ...
    if (value !== '*') {
      ...
      // let match = /^[^\\@!/\[,$*+?.^{}(|)\]]+/.exec(input.slice(state.index + 1));
      let match = /^\w+/.exec(input.slice(state.index + 1));
      if (match) {
        value += match[0];
    ...
    if (prev.type === 'star') {
      ...
      if (prior.type === 'slash' && prior.prev.type !== 'bos' && next === '/') {
        ...
        prev.type = 'globstar';
        prev.output = `${globstar(opts.dot)}${SLASH_LITERAL}|${SLASH_LITERAL}|$)`;
        prev.value += value;
      ...
      if (prior.type === 'bos' && next === '/') {
        prev.type = 'globstar';
        prev.value += value;
        prev.output = `(?:^|${SLASH_LITERAL}|${globstar(opts.dot)}${SLASH_LITERAL})`;
        state.output = prev.output;
        state.consumed += value + advance();
        push({ type: 'slash', value, output: '' });
        continue;
