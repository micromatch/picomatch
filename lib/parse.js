...
const parse = (input, options) => {
  ...
  input = utils.createPrefix(input, state);
  len = input.length;
  ...
  const push = tok => {
    if (prev.type === 'globstar') {
      ...
      let isExtglob = tok.extglob === true || (extglobs.length && (tok.type === 'pipe' || tok.type === 'paren'));
  ...
  while (!eos()) {
    ...
    /**
     * Angle backets - for <path> variables
     */

    if (value === '<' && opts.variables && (state.parens === 0 || prev.value !== '?')) {
      let rest = remaining();
      let closeIndex = rest.indexOf('>');

      if (closeIndex > 0) {
        let prop = rest.slice(0, closeIndex);
        let output = opts.variables[prop];

        if (output) {
          state.index += closeIndex + 1;
          value += rest.slice(0, closeIndex + 1);
          output = utils.toPosixSlashes(output);

          if (output[0] === '!') {
            output = `!(${output.slice(1)})`;
          }

          let result = parse(output, { ...opts, wrap: false, contains: true });
          let token = { type: 'variable', value, raw: output, output: result.output };
          push(token);

          if (token.output.slice(-1) === '/' && peek() === '/') {
            consume('/', 1, false);
          }

          if (typeof opts.onVariable === 'function') {
            opts.onVariable(token, state);
          }
          continue;
        }
      }
    ...
    if (value === '+') {
      ...
      if (prev && (prev.type === 'bracket' || prev.type === 'paren' || prev.type === 'brace')) {
        let output = prev.value === '(' ? PLUS_LITERAL : value;
  ...
  if (opts.wrap !== false) {
    state.output = utils.wrapOutput(state.output, state, options);
  }
...
parse.fastpaths = (input, options) => {
  ...
  let state = { negated: false, prefix: '' };
  ...
  let output = utils.createPrefix(input, state);
  ...
  if (source) {
    state.output = utils.wrapOutput(source, state, options);
  }
