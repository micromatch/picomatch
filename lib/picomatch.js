...
picomatch.makeRe = (input, options) => {
  let makeRe = pattern => {
    let flags = options && (options.flags || (options.nocase ? 'i' : ''));
    let state = picomatch.parse(pattern, options);
    return toRegex(state.source, flags, options);
  };
...
function toRegex(value, flags, options) {
  try {
