...
const picomatch = (glob, options, returnState = false) => {
  ...
  const matcher = (input, returnObject = false) => {
    let { isMatch, output } = picomatch.test(input, regex, options, { glob, posix });
    let result = { glob, regex, posix, input, output, isMatch, state };
...
picomatch.test = (input, regex, options, { glob, posix } = {}) => {
  ...
  let isMatch = input === glob;
  let output = (isMatch && format) ? format(input) : input;
