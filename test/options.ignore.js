...
describe('options.ignore', () => {
  it('should not match ignored patterns', () => {
    assert(
      isMatch('a+b/src/glimini.js', 'a+b/src/*.js', { ignore: ['**/f*'] })
    );
    assert(
      !isMatch('a+b/src/glimini.js', 'a+b/src/*.js', { ignore: ['**/g*'] })
    );
  ...
  it('should filter out ignored patterns', () => {
    ...
    assert.deepStrictEqual(match(globs, '**/*/[b-z]', opts), [
      'b/b/b',
      'b/b/c',
      'c/c/c',
      'e/f/g',
    ...
    assert.deepStrictEqual(match(globs, '*', { ignore: '**/a', dot: true }), [
      '.a',
      'b'
    ]);
    ...
    assert.deepStrictEqual(match(['foo.js', 'a/foo.js'], '**/foo.js'), [
      'foo.js',
      'a/foo.js'
    ]);
    assert.deepStrictEqual(
      match(['foo.js', 'a/foo.js'], '**/foo.js', { dot: true }),
      ['foo.js', 'a/foo.js']
    );
    ...
    assert.deepStrictEqual(match(negations, '**', { ignore: ['*/b', '*/a'] }), [
      'a/c',
      'a/d',
      'a/e',
      'b/c'
