...
describe('dotfiles', () => {
  describe('normal', () => {
    it('should not match dotfiles by default:', () => {
      ...
      assert.deepStrictEqual(match(['a/b', 'a/.b', '.a/b', '.a/.b'], '**'), [
        'a/b'
      ]);
  ...
  describe('leading dot', () => {
    it('should match dotfiles when a leading dot is defined in the path:', () => {
      assert.deepStrictEqual(match(['a/b/c/.dotfile.md'], '**/.*'), [
        'a/b/c/.dotfile.md'
      ]);
      assert.deepStrictEqual(match(['a/b/c/.dotfile.md'], '**/.*.md'), [
        'a/b/c/.dotfile.md'
    ...
    it('should use negation patterns on dotfiles:', () => {
      assert.deepStrictEqual(match(['.a', '.b', 'c', 'c.md'], '!.*'), [
        'c',
        'c.md'
      ]);
      assert.deepStrictEqual(match(['.a', '.b', 'c', 'c.md'], '!.b'), [
        '.a',
        'c',
        'c.md'
      ]);
    ...
    it('should match dotfiles when there is a leading dot:', () => {
      ...
      assert.deepStrictEqual(
        match(['a/b', 'a/.b', '.a/b', '.a/.b'], '**', opts),
        ['a/b', 'a/.b', '.a/b', '.a/.b']
      );
      assert.deepStrictEqual(
        match(['a/b', 'a/.b', 'a/.b', '.a/.b'], 'a/{.*,**}', opts),
        ['a/b', 'a/.b']
      );
      assert.deepStrictEqual(
      ...
      assert.deepStrictEqual(match(['.dotfile'], '.dotfile', opts), [
        '.dotfile'
      ]);
      assert.deepStrictEqual(match(['.dotfile.md'], '.*.md', opts), [
        '.dotfile.md'
      ]);
    });
    ...
    it('should match dotfiles when there is not a leading dot:', () => {
      const opts = { dot: true };
      assert.deepStrictEqual(match(['.dotfile'], '*.*', opts), ['.dotfile']);
      assert.deepStrictEqual(match(['.a', '.b', 'c', 'c.md'], '*.*', opts), [
        '.a',
        '.b',
        'c.md'
      ]);
      ...
      assert.deepStrictEqual(match(['a/b/c/.verb.md'], '**/*.md', opts), [
        'a/b/c/.verb.md'
      ]);
  ...
  describe('options.dot', () => {
    it('should match dotfiles when `options.dot` is true:', () => {
      ...
      assert.deepStrictEqual(match(['.dotfile'], '*.*', { dot: true }), [
        '.dotfile'
      ]);
      assert.deepStrictEqual(match(['.dotfile'], '*.md', { dot: true }), []);
      assert.deepStrictEqual(match(['.dotfile'], '.dotfile', { dot: true }), [
        '.dotfile'
      ]);
      assert.deepStrictEqual(match(['.dotfile.md'], '.*.md', { dot: true }), [
        '.dotfile.md'
      ]);
      ...
      assert.deepStrictEqual(
        match(['a/b/c/.dotfile.md'], '**/.*', { dot: false }),
        ['a/b/c/.dotfile.md']
      );
      assert.deepStrictEqual(
        match(['a/b/c/.dotfile.md'], '**/.*.md', { dot: false }),
        ['a/b/c/.dotfile.md']
      );
      assert.deepStrictEqual(
        match(['a/b/c/.dotfile.md'], '*.md', { dot: false }),
      ...
      assert.deepStrictEqual(
        match(['a/b/c/.verb.md'], '**/*.md', { dot: true }),
        ['a/b/c/.verb.md']
      );
      assert.deepStrictEqual(match(['d.md'], '*.md', { dot: true }), ['d.md']);
      assert.deepStrictEqual(match(fixtures, 'a/*/b', { dot: true }), [
        'a/c/b',
        'a/.d/b'
      ]);
